...
- [ ] **优化本地 RAG 检索速度（冷启动 + 候选重排）**
  - **现状**：当前 `rag/` 链路已经接入本地 embedding、BM25 和 reranker，但首次调用明显偏慢。主要耗时点包括：BM25 在新进程里重建整套 tokenized corpus、embedding / reranker 模型冷启动，以及召回候选池偏大后带来的重排成本。这种形态适合实验，不适合后续直接作为 Agent 高频 tool 调用。
  - **优化思路**：后续应将 RAG 工具改为长生命周期进程或服务化形态，避免每次请求都重新加载模型；将 BM25 从“启动时全量重建”改为“离线构建索引并本地加载”；重新评估 `candidate multiplier`，在召回质量可接受的前提下缩小 reranker 输入规模；必要时增加查询级缓存，降低重复查询成本。
